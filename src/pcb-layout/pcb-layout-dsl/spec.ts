...
- When approximate internal topology is known, use block(..., { localLayout: { U1: { x: 0, y: 0, rotate: 0 }, L1: { x: 6, y: 0 } } }). localLayout is sparse: only listed direct block members are seeded. Unlisted members are placed normally around the seeded rigid macro. Coordinates are approximate block-local positions, not absolute board coordinates. The legalizer preserves the requested topology/order and moves only seeded members involved in a real footprint collision/clearance violation. A component listed in localLayout cannot also use fixed(), edgeMount(), or edgePlace().
...
- Do not leave a movable passive-only block at board level without placement intent. Make it a satellite with placement: "satellite", attachTo, and preferably anchor: pin(...) on the parent, or connect one of its components to an external component with near(), veryNear(), or criticalPair(). allowDisconnected only permits the block topology; it does not attach the block spatially.
...
- fixed() is allowed for any component role. Use it only when the component has a genuinely known absolute/mechanical position; do not use fixed() merely to improve ordinary electrical placement. Prefer blocks, localLayout, satellites, anchors, near/veryNear, criticalPair, bypass, or capCluster for normal placement. A component explicitly listed in localLayout cannot also use fixed(), edgeMount(), or edgePlace().
...
- Ordinary non-GND connectivity already contributes weak board-level attraction automatically. Nets spanning more than 8 placement primitives are omitted from this weak affinity, ignoredSignals are skipped, and power nets use only 0.1x attraction weight. Do not add near()/veryNear() for every ordinary net; reserve explicit hints for real physical intent.
...
- After the normal placement score produces a shortlist, the native placer applies a bounded Micro-A* routability correction to board/block candidates. Explicit critical/high-priority route jobs are evaluated first; a bounded, spatially representative sample of ordinary signal nets is evaluated afterward. Temporary estimated routes remain occupied while evaluating later jobs for the same candidate, so congestion can affect the score. This is only a placement/routability estimator: it does not create PCB traces and does not replace the EasyEDA router.
...
- Do not write routing rules in this DSL. Micro-A* is an internal bounded routability estimator used only while scoring placement candidates. Actual routing, DRC classes, copper polygons, and via stitching are handled by the EasyEDA v3 client after server-side placement.
